 ...
 *
 * Nos dois casos o destino final **não** é o backbuffer: é o FBO de página
 * compartilhado (`gl.frame.target`, ver `engine/frame.ts`). Quem passa por
 * aqui não precisa chamar `setRenderTarget` — este módulo devolve o FBO de
 * página ligado quando termina — e quem desenha direto (sem composite) conta
 * com ele já ligado por `frame.beginFrame()`.
 *
 * ⚠️ Este módulo **troca de render target no meio do quadro** (duas camadas,
 * dois FBOs, um quad final). Uma seção que passa por aqui precisa recortar o
 * próprio retângulo com `gl.frame.setScissorCss()`, nunca com
 * `renderer.setScissor()` — a armadilha está documentada em `frame.ts`.
 */
