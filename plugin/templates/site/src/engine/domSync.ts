 ...
 *
 * Vale mantê-la em produção, e não só na página de dev: é o que transforma o
...
export interface DomSync {
   ...
   *
   * É o que permite uma seção derivar progresso de scroll por bloco (uma grade
   * de 16 fichas, por exemplo) sem transformar 16 planos em 32 leituras de
...
export function createDomSync(host: DomSyncHost): DomSync {
  ...
  /**
   * Índice para `rectOf`. Um `Map` em vez de varrer `entries`: uma seção em
   * grade consulta um retângulo por ficha, todo quadro, e uma busca linear
