 ...
 * produção).
 *
 * Cada seção continua dona de um retângulo e recorta o próprio `scissor`
 * (regra do canvas, `main.ts`) — a posse por retângulo não muda. O que este
 * módulo muda é o **alvo**: em vez do backbuffer, toda seção escreve neste
 * FBO (`target`), e só o passe de grade, no fim do quadro, escreve no
 * backbuffer de verdade. Uma imagem, um dono de cada pixel, um único ponto
 * de saída — é o que fecha a emenda entre seções em vez de exigir uma
 * correção por seção.
 *
 ...
 * aplica o bloom a partir do FBO e desenha o quad de grade no backbuffer.
 * Entre as duas fases, cada seção lê o FBO como se fosse "a tela": quem
 * desenha direto **não chama `setRenderTarget`** (o FBO já está ligado por
 * `beginFrame()`), e quem passa por `composite.ts` recebe o FBO de volta
 * ligado quando ele termina.
 */
 ...
 * Abaixo disso é fundo, não luz: "florescer" ali só clarearia as chapas que o
 * dither existe para manter limpas. Este e os cinco números abaixo são
 * defaults conservadores do passe de grade — ajuste-os ao brief do site (é a
 * única parte deste arquivo que é decisão de imagem, não de motor).
 */
 ...
 * vale a pena repetir o padrão em toda cena: um erro de digitação num nome de
...
export interface Frame {
  ...
  beginFrame(): void;
  /**
   * Recorta, em px CSS, o que uma seção escreve no FBO de página quando ela
   * troca de render target no meio do quadro (é o caso de quem passa por
   * `composite.ts`).
   *
   * ⚠️ **Armadilha medida.** `renderer.setViewport()`/`setScissor()` são
   * **ignorados** por quem troca de render target no meio do quadro — como
   * `composite.render()`, que usa dois FBOs próprios antes do quad final. A
   * cada troca o three reaplica o scissor **do alvo que acabou de ser
   * ligado**, não o que ficou em estado solto do renderer, e o recorte some
   * sem erro nenhum: a seção passa a desenhar sobre as vizinhas.
   * `WebGLRenderTarget.scissor` é o mecanismo do próprio three para isto —
   * sobrevive a qualquer troca de alvo intermediária porque mora no objeto do
   * alvo. Regra prática: **trocou de target no quadro → use isto**; nunca
   * trocou → `renderer.setScissor()` continua certo e é mais barato.
   */
...
function createFullscreenMesh(material: RawShaderMaterial): Mesh {
  ...
  geometry.setAttribute(
    'position',
    // Mesmo triângulo de tela cheia do composite — três componentes: o three
    // tira a bounding sphere de `position`, e um atributo sem Z dá raio NaN.
...
export function createFrame(host: FrameHost): Frame {
  ...
  // RGBA8, não RGBA16F — decisão medida no protótipo que originou este
  // template. No tier `high`, um FBO de página RGBA16F custava 13,72 ms de
  // mediana de GPU (o teto de um quadro a 60 fps é 16,67 ms, e o aceite era
  // ~13,5 ms); em RGBA8 a mediana caiu para 9,8–11,5 ms. A diferença é banda
  // passante de escrita de cor em **cada** técnica que escreve aqui todo
  // quadro (RGBA16F dobra os bytes por pixel de todas elas), não o passe de
  // grade em si. Isso não reabre o banding que o dither existe para matar:
  // toda cena que escreve aqui termina o próprio shader em `linearToSrgb`
  // (o mesmo valor que iria para um backbuffer de 8 bits), então o FBO nunca
  // teve precisão real além de 8 bits — RGBA16F só guardaria o mesmo valor
  // quantizado num tipo maior. Só troque por `HalfFloatType` se as suas cenas
  // passarem a escrever **luz linear** aqui (e aí meça de novo).
  ...
  const target = new WebGLRenderTarget(MIN_TARGET_PX, MIN_TARGET_PX, {
    ...
    // Compartilhado entre todas as seções: uma cena 3D precisa de
