...
/**
 * O passe de grade final (VI.3+VI.4 do catálogo, em produção): três shaders que
 * transformam o FBO de página — a montagem inteira das 8 seções, em um retrato
 * único — de "render de browser" em "imagem revelada".
 *
 * Todos os três compartilham o mesmo vertex de tela cheia (`glsl.ts`), então
 * este módulo só exporta fragments.
 *
 * ── POR QUE O FBO NÃO É HDR DE VERDADE ──────────────────────────────────────
 * Os shaders que já existem (limalha, relevo, papel do catálogo, máscara do
 * composite) sempre terminam em `linearToSrgb` — nasceram para escrever direto
 * na tela, e dois deles (`points.ts`, `relight.ts`) estão fora do escopo desta
 * tarefa. Reescrevê-los para devolver luz linear duplicaria o trabalho e
 * arriscaria o contraste medido. Este passe portanto opera em espaço **de
 * exibição** (os mesmos números que a tela receberia, antes só do encode
 * final) — bloom aqui é um brilho cosmético sobre a imagem já revelada, não
 * uma soma de radiância física. É a mesma simplificação que motores de jogo
 * fazem quando o pipeline upstream não é HDR: o ganho (banding morto, sem
 * emenda entre seções) supera a pureza que se perderia tentando linearizar
 * shaders que não pediram para ser tocados. Consequência medida: como não há
 * faixa acima de 1.0 para preservar, o FBO de página é `RGBA8`, não
 * `RGBA16F` — ver o comentário de `createFrame` em `engine/frame.ts`.
 *
 ...
 * curto que o mip-chain (menos alcance), mas o suficiente para o metal quente
 * continuar lendo como quente.
 */

/**
 * O passe final: FBO de página -> tela. Ordem fixa, a mesma do briefing:
